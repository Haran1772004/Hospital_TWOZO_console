...
public class UserLFImpl implements UserLF {
  private static final Map<String, User> users = new ConcurrentHashMap<>();

  static {
    users.put(
        "admin",
        new User("admin", PasswordUtil.hashPassword("admin123"), "ADMIN", 0, AccountStatus.ACTIVE));

    users.put(
        "receptionist",
        new User(
            "receptionist",
            PasswordUtil.hashPassword("reception123"),
            "RECEPTIONIST",
            0,
  ...
  public void removeUser(String username) {
    if (username != null) {
      users.remove(username.trim().toLowerCase());
    }
  }
}
