...
public class PrescriptionLFImpl implements PrescriptionLF {
  ...
  public void addPrescription(Prescription p) {
    if (p == null
        || p.getRecordId() <= 0
        || !ValidationUtil.isNonBlank(p.getMedicineName())
        || !ValidationUtil.isNonBlank(p.getDosage())
        || !ValidationUtil.isNonBlank(p.getDuration())) {
  ...
  @Override
  public List<Prescription> getPrescriptionsByRecord(int id) {
    return prescriptions.stream().filter(p -> p.getRecordId() == id).toList();
  }
}
